...
k () {
  ...
  # ----------------------------------------------------------------------------
  # default colors
  K_COLOR_DI="0;34"  # di:directory (Azul)
  K_COLOR_LN="0;35"  # ln:symlink
  K_COLOR_SO="0;32"  # so:socket (Verde)
  K_COLOR_PI="0;33"  # pi:pipe
  # K_COLOR_EX="0;31"  # ex:executable (vermelho)
  K_COLOR_EX="0;33"  # ex:executable (Amarelo)
    ...
    do
      ...
      if [[ "$o_no_vcs" != "" ]]; then
        ...
        if (( INSIDE_WORK_TREE == 0 )); then
          ...
          # Status GIT de cada arquivo listado
          # WORK -> STAGE -> LOCAL -> REMOTE
          STATUS=${STATUS[1,2]}
          
          # (Vermelho) WORK New - Untracked (crei o arquivo)
          if [[ $STATUS == '??' ]]; then REPOMARKER=$'\e[31m+ working    \e[0m';
          # (Amarelo) WORK changed - Tracked & Dirty (alterado)
          elif [[ $STATUS == ' M' || $STATUS == 'AM' || $STATUS == 'MM' ]]; then REPOMARKER=$'\e[38;5;214m+ working    \e[0m'; 
          # elif [[ $STATUS == ' M' || $STATUS == 'AM' || $STATUS == 'MM' ]]; then REPOMARKER=$'\e[33m+ work  \e[0m'; 
          
          # Ciano (Azul claro) STAGE (add) - Tracked & Dirty & Added  ou apenas Added 
          elif [[ $STATUS == 'M ' ]]; then REPOMARKER=$'\e[0;32m● staging    \e[0m'; 
          elif [[ $STATUS == 'A ' ]]; then REPOMARKER=$'\e[0;32m● staging    \e[0m'; 


          elif [[ $STATUS == '!!' ]]; then REPOMARKER=$'\e[38;5;238m● ignored    \e[0m'; # Ignored (Marron) gitignore
          
          else                             REPOMARKER=$'\e[38;5;082m✓ Repository \e[0m'; # Good (ok verde) LOCAL (commit)
          fi
        fi
      fi
